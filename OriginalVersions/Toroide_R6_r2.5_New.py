...
seam_pad = 0.0    # Å  → grosor de "banda" que recortamos para evitar duplicados en la costura
...
# MODIFICACIÓN: Se calcula nx para que la longitud del bloque sea
# ligeramente MAYOR que la circunferencia objetivo.
# Esto asegura que tenemos suficientes átomos para cubrir todo el toroide.
...
# MODIFICACIÓN: La longitud del cilindro ahora es exactamente Lx, que es L_target.
# Ya no es necesario recalcularla desde las posiciones.
L = Lx
...
# MODIFICACIÓN: Usamos L_target como la longitud para el mapeo angular.
# Esto asegura que cuando s llega a L_target, theta llega exactamente a 2π,
# cerrando el toroide a la perfección geométrica.
L = L_target  # La longitud efectiva para el mapeo es la ideal, no la real del bloque.
theta = 2.0 * np.pi * (s / L)
...
# ==========================
# MODIFICACIÓN: El filtro ahora elimina los átomos que están más allá
# de la longitud de la circunferencia objetivo (L_target).
seam_mask = s < L  # Usar '<' en lugar de '<=' para evitar que el punto final (s=L) se duplique con el inicial (s=0)
...
# 7) Guardar
# ==========================
write("Li2O2_toroideNuevo.xyz", toroid)
print("Guardado: Li2O2_toroideNuevo.xyz")
